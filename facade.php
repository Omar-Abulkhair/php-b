...
function createDbUsers(){
    ...
    echo '<br>insert: Admin - token: ' . $ret = $db->newUser('admin', 'adminadmin','Bill Rocha', 'Author & Administrator','prbr@ymail.com', 10);
    echo '<br>insert: User  - token: ' . $ret = $db->newUser('user', 'adminadmin', 'User Default', 'Other User', 'Tel.: +55 21 9 8795 0673', 2);
    echo '<br>insert: Guest - token: ' . $ret = $db->newUser('guest', 'adminadmin', 'I\'m a Guest User');
    ...
    <?php if($name == 'terminal'){ ?>
        <div class="topbar">
            ...
            <form method="get" action="<?php echo URL;?>">
                <input type="text" id="command" name="command" placeholder="type a command here..." value="" autofocus/>
            </form>
        </div>
